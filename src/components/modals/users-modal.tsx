...
export const UsersModal = ({ isOpen, close }: Props) => {
  const { data: users, getData: getUsers, loading: usersLoading } = useGet<User[]>({ manual: true })
  ...
  return (
    <>
      <Modal show={isOpen} size="2xl" popup onClose={() => close()}>
        ...
        <Modal.Body className="relative w-full h-full items-center block bg-white border border-gray-100 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-800 dark:hover:bg-gray-700">
          {users && (
          ...
          {usersLoading && (
            <div role="status" className="flex w-full justify-center py-4">
              <svg
                aria-hidden="true"
